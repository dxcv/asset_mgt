...
#        self.pct = self.data.pct_change().dropna() 
        ...
        data = cursor.fetchall()
        data = pd.DataFrame(data,columns = [each[0] for each in cursor.description])
...
class BLDataSourceStyle(DataSource):
    '''
    BL行业数据源.
    '''
    def pre_load(self):
...
class LocalBLDataSource():
    ...
    def __init__(self):
        self.calendar = Calendar(path = 'D:\Work\calendar')
        self.history_data = pd.read_excel('.//data//industry_data.xlsx',parse_date = 'date').set_index('date')
        self.market_cap = pd.read_excel('.//data//industry_market_cap.xlsx',parse_date = 'date').set_index('date')
